...
class Logger (object):
    ...
    def __init__ (self):
        ...
        self._option.BGPSEC        = command.log.enable and (command.log.all or command.log.BGPSEC)
    ...
    def BGPSEC (self, message, recorder='info'):
        level = short(recorder)
        if self._option.BGPSEC and getattr(syslog,'LOG_%s' % level) <= self.level:
            getattr(self,recorder.lower())(self,message,'BGPSEC')
        self.pdb(recorder)
